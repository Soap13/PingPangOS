...
;=========================================================	
  ...
  flush: 
	...
	.loop_read:	 
		 ...
		 xor	esi, esi
	     mov	cx, word [edi + 2Ch]; ┓ ecx <- pELFHdr->e_phnum
	     movzx	ecx, cx					; ┛
	     mov	esi, [edi + 1Ch]	; esi <- pELFHdr->e_phoff
	     add	esi, edi		; esi <- OffsetOfKernel + pELFHdr->e_phoff
   .Begin:
	     mov	eax, [esi + 0]
	     cmp	eax, 0				; PT_NULL
	     jz	.NoAction
	     ...
	     mov	eax, [esi + 04h]		;	┃
	     add	eax, edi	;	┣ ::memcpy(	(void*)(pPHdr->p_vaddr),
